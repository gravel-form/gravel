...
exports.FixedObjectMw = function (props) {
    ...
    if (!properties)
        return next(props);
    return (React.createElement(React.Fragment, null, Object.keys(properties).map(function (key) { return (React.createElement(MiddlewareComponent, __assign({ key: key }, props, { schema: properties[key], data: Object.hasOwnProperty.call(data, key) ? data[key] : undefined, onChange: function (value) {
            var _a;
...
exports.LocalRefMw = function (props) {
    ...
    if (!child || child.schema === schema)
        return next(nextProps);
    return React.createElement(MiddlewareComponent, __assign({}, nextProps, { schema: child.schema, schemaPath: child.path }));
};
...
exports.FormCore = function (props) {
    ...
    if (!middlewares || !schema)
        return null;
    var Composed = React.useMemo(function () { return (Array.isArray(middlewares) ? compose_1.default(middlewares) : middlewares); }, [
        middlewares,
    ]);
    return (React.createElement(Composed, { schema: schema, onChange: onChange || noop, data: data, schemaPath: [], dataPath: [], parent: null, next: Null, MiddlewareComponent: Composed, formProps: props }));
};
