...
function useAdditional(props, AdditionalItemTemplate) {
    ...
    var bindChildProps = function (i) { return (__assign(__assign({}, props), { onChange: function (value) { return onChange(__spreadArrays(data.slice(0, i), [value], data.slice(i + 1))); }, schema: itemSchema, data: data[i], schemaPath: __spreadArrays(schemaPath, ['items', i]), dataPath: __spreadArrays(dataPath, [i]), parent: props })); };
    var arrayBody = [];
...
exports.FixedObjectArrayMw = function (props) {
...
exports.FormCore = function (props) {
    var schema = props.schema, data = props.data, middlewares = props.middlewares, onChange = props.onChange, rest = __rest(props, ["schema", "data", "middlewares", "onChange"]);
